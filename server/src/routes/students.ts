...
function listStudents(db: ReturnType<typeof getDb>) {
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      petStageLabel: r.pet_id ? levelLabelOf(getDb(), speciesLike, r.pet_exp ?? 0, thresholds) : null,
...
function getStudentDetail(db: ReturnType<typeof getDb>, studentId: string) {
  ...
  if (petRow) {
    ...
    pet = {
      ...
      stageLabel: levelLabelOf(db, species, refreshed.exp, thresholds),
      stageLabels: globalLevelNames(db),
      thresholds,
